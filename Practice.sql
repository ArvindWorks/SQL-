...
WHERE DIFF > 7;
--------------------------------------------------------------------------------------------------------------------------------------------------------
--WAQ TO GET THE COMPANY ID WHO HAVE ATLEAST 2 USERS WHO SPEAKS BOTH ENGLISH AND GERMAN
...
WITH CTE AS(
              SELECT COMPANY_ID,USER_ID,COUNT(*) CT FROM COMPANY_USER
              WHERE LANGUAGE IN ('ENGLISH','GERMAN')
              GROUP BY COMPANY_ID,USER_ID
              HAVING COUNT(*)>1
         )

SELECT COMPANY_ID,COUNT(*) CT
FROM CTE GROUP BY COMPANY_ID
HAVING COUNT(*)>1
------------------------------------------------------------------
--WAQ TO FIND THE START AND END DATE OF USERS WHERE THEY LOGIN CONSECTIVE FOR MORE THAN 2 DAYS
CREATE TABLE Logins (
    UserID INT,
    LoginDate DATE
);
...
-- Insert sample data
INSERT INTO Logins (UserID, LoginDate) VALUES 
(1, '2023-01-01'),
(1, '2023-01-02'),
(1, '2023-01-03'),
(1, '2023-01-05'),
...
SELECT *,
DAY(LOGINDATE) D,
ROW_NUMBER() OVER(PARTITION BY USERID ORDER BY USERID) RN,
DAY(LOGINDATE) - ROW_NUMBER() OVER(PARTITION BY USERID ORDER BY USERID) DIFF
FROM LOGINS
)

SELECT USERID, MIN(LOGINDATE) START_DATE,
MAX(LOGINDATE) END_DATE
FROM CTE
